...
# Locate Numbers adjacent to "*" symbols
LINE_NUMBER=0
while read LINE; do
	INDEX=0
	BEGIN=0
	while [[ "$INDEX" != -1 ]]; do
		...
		if [[ "$INDEX" != -1 ]]; then
			#Classify number
			for ((SYMBOL_INDEX=0; SYMBOL_INDEX<${#SYMBOLS[@]}; SYMBOL_INDEX++ )); do
				SYMBOL_ENTRY=${SYMBOLS[$SYMBOL_INDEX]}
				read SYMBOL SYMBOL_X SYMBOL_Y NUMBERS <<<"$SYMBOL_ENTRY"
				if [[ "$SYMBOL_X" -ge $((INDEX-1)) && \
				      "$SYMBOL_X" -le $BEGIN && \
				      "$SYMBOL_Y" -ge $((LINE_NUMBER-1)) && \
				      "$SYMBOL_Y" -le $((LINE_NUMBER+1)) ]]; then
					SYMBOLS[$SYMBOL_INDEX]+=" $MATCH_STRING"
				fi
			done
		fi
	done
...
for SYMBOL_ENTRY in "${SYMBOLS[@]}"; do
	...
	NUMBERS=($NUMBERS)
	if [[ ${#NUMBERS[@]} == 2 ]]; then
		ACCUMULATOR=$((ACCUMULATOR+(${NUMBERS[0]}*${NUMBERS[1]})))
	fi
done
echo "$ACCUMULATOR"
